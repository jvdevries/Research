...
namespace DevUtil.Dev_Testing.CompatibilityTesters.IO.FileRead
{
    public class SyncTester : TesterBase
    {
        public SyncTester(DirectoryInfo testDir, FileOptions flag, int fileSize, params int[] fileSizes) : base(GetTestDimension(testDir, fileSize, fileSizes), GetTest(testDir, flag))
        {
        }
        ...
        {
            Exception SyncRead(int fileSizeVar, int fsBufferSizeVar, int rBufferSizeVar)
            {
                ...
                try
                {
                        ...
                        FileShare.None))
                    {
                        ...
                        do
                        {
                            ...
                            bytesRedThisPass = testFile.Read(readBuffer, 0, bytesToReadThisPass);
            ...
            return Test.Create((Func<int, int, int, Exception>) SyncRead);
        }
    }
}