...
volatile int X, Y, r1, r2, r3, r4 = 0;
int counter1, counter2 = 0;
volatile int loopRuns = 0;
...
int main(int argc, char* argv[]) 
{
	if (argc != 4)
	{
		cout << "Call with arguments maxRuns, printPer & loopRuns" << endl;
		exit(0);
	}

	int maxRuns = stoi(argv[1]);
	int printPer = stoi(argv[2]);
	loopRuns = stoi(argv[3]);
	
	auto start = clock();
	for (int run = 1; run <= maxRuns; run++)
	{
		X = Y = r1 = r2 = r3 = r4 = counter1 = counter2 = 0;
		ready = false;
